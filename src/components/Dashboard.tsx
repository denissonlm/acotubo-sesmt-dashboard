...
import { AlertCircle, TrendingUp, Calendar, Filter as FilterIcon, Upload, Printer, ShieldCheck } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ accidents, selectedYears, onYearsChange, onReset, onPrint }) => {
      ...
      <header className="header no-print">
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          <div className="filter-group">
            <label style={{ color: 'white' }}>Triênio de Referência</label>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <input 
                type="number" 
                value={selectedYears[0]} 
                onChange={(e) => onYearsChange([Number(e.target.value), Number(e.target.value) + 1, Number(e.target.value) + 2])}
                style={{ width: '80px', padding: '0.4rem', borderRadius: '4px', border: 'none' }}
              />
              <span style={{ alignSelf: 'center', color: 'white' }}>+2 anos</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1rem' }}>
      ...
      <div className="grid-main">
        ...
        <aside className="insights-panel">
          <div style={{ marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '1.4rem', fontWeight: 900, color: 'var(--text)', margin: 0, letterSpacing: '-0.02em' }}>Períodos de <span style={{ color: 'var(--primary)' }}>Atenção</span></h2>
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.25rem', fontWeight: 500 }}>Padrões identificados na comparação trienal</p>
          </div>
